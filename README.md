...
- **Places I've Been:** the globe icon next to the logo opens a region picker (Europe, Asia, Africa, North America, South America, Oceania, each showing a % of countries ticked off), each with a rotating, colour-coded 3D globe up top, tap a country on it to jump straight there, or drag/zoom to explore (auto-spin stops as soon as you interact). Tick Been or Want to Go for any country; any country on an upcoming trip's itinerary shows as Going automatically, and flips to Been once that trip is over. Filter the whole page (and the globe) down to just Been, Want to Go or Going. Each country shows its capital, language and currency symbol. Includes England, Scotland, Wales and Northern Ireland as their own entries, not lumped into a single United Kingdom.
