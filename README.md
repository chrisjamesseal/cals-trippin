...
- **Places I've Been:** **Places** on the bottom bar opens it, and three counters at the top (Places I've Been, Want to Go, Countries) filter the page to that list in one tap. It starts on a region picker: an "All" card plus Europe, Asia, Africa, North America, South America and Oceania, each with its own emoji and how many of its countries you've ticked off. Tick Been or Want to Go for any country; any country on an active trip's itinerary shows as Been automatically, no need to tick it yourself. Filter the whole page down to just Been or Want to Go, with a live count next to each filter. Each country shows its capital, language and a "Search Flights" link out to Google Flights (on an iPhone with the app on your home screen this hands over to Safari itself rather than the cut-down in-app browser, so your Google login and the Google Flights app still work). Includes England, Scotland, Wales and Northern Ireland as their own entries, not lumped into a single United Kingdom.
